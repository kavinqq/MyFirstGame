...
public class ZombieAnimator extends Animator {
    ...
    public ZombieAnimator(int type, State state) {
        ...
        // 初始化圖片
        img = SceneController.getInstance().imageController().tryGetImage(new Path().img().zombies().zombieNormal());
    ...
    public void paint(Global.Direction dir, int left, int top, int right, int bottom, Graphics g) {
        ...
        g.drawImage(img, left, top, right, bottom,
                (type % 4) * 48 * 3 + state.getArr()[walkCount] * 48,
                (type / 4) * 72 * 4 + dir.getValue() * 72,
                (type % 4) * 48 * 3 + state.getArr()[walkCount] * 48 + 48,
                (type / 4) * 72 * 4 + dir.getValue() * 72 + 72,
                null);
    }
